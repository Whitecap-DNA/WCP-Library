...
class TestGetSiteMetadata:
    def test_returns_site_json_and_calls_correct_url(self):
        with patch("wcp_library.graph.sharepoint.requests.get") as mock_get:
            mock_get.return_value = _ok_json({"id": SITE_ID, "displayName": "DataOps"})
...
class TestGetDrives:
    def test_returns_all_drives(self):
        ...
        with patch(
            "wcp_library.graph.sharepoint.requests.get", return_value=page
        ) as mock_get:
            result = sharepoint.get_drives(HEADERS, SITE_ID)
            assert result == [{"id": "d1", "name": "Documents"}]
            assert mock_get.call_count == 1
            called_url = mock_get.call_args[0][0]
            assert called_url == f"https://graph.microsoft.com/v1.0/sites/{SITE_ID}/drives"
    ...
    def test_returns_none_on_request_exception(self):
        with patch(
            "wcp_library.graph.sharepoint.requests.get", side_effect=_http_error()
        ):
...
class TestGetDriveIdByName:
    def test_returns_matching_drive_id(self):
        ...
        with patch(
            "wcp_library.graph.sharepoint.requests.get",
...
class TestListFolder:
    def test_returns_items_for_root_when_path_is_slash(self):
        ...
        with patch(
            ...
            called_url = mock_get.call_args[0][0]
            assert called_url.endswith(f"/sites/{SITE_ID}/drive/root/children")
...
class TestUploadFile:
    def test_uploads_bytes_with_default_conflict_behavior(self):
        ...
        with patch(
            "wcp_library.graph.sharepoint.requests.put",
            ...
            assert mock_put.call_args.kwargs["data"] == b"file-bytes"
            assert mock_put.call_args.kwargs["headers"] == HEADERS
    ...
    def test_custom_conflict_behavior_appears_in_url(self):
        with patch(
            ...
            called_url = mock_put.call_args[0][0]
            assert called_url.endswith("@microsoft.graph.conflictBehavior=replace")
    ...
    def test_base64_string_is_decoded_before_upload(self):
        ...
        with patch(
            ...
            assert mock_put.call_args.kwargs["data"] == b"hello world"
    ...
    def test_bytearray_is_converted_to_bytes(self):
        with patch(
            ...
            assert mock_put.call_args.kwargs["data"] == b"abc"

    def test_returns_none_on_request_exception(self):
        with patch(
            "wcp_library.graph.sharepoint.requests.put", side_effect=_http_error()
        ):
            assert (
                sharepoint.upload_file(
...
class TestMoveFile:
    def test_patches_with_parent_reference_payload(self):
        ...
        with patch(
            "wcp_library.graph.sharepoint.requests.patch",
            return_value=_ok_json(response_payload),
        ) as mock_patch:
            ...
            assert (
                mock_patch.call_args.kwargs["headers"]["Content-Type"]
                == "application/json"
            )
    ...
    def test_returns_none_on_request_exception(self):
        with patch(
            "wcp_library.graph.sharepoint.requests.patch", side_effect=_http_error()
        ):
            assert (
...
class TestCopyFile:
    def test_posts_with_parent_reference_payload(self):
        with patch(
            "wcp_library.graph.sharepoint.requests.post",
    ...
    def test_returns_none_on_request_exception(self):
        with patch(
            "wcp_library.graph.sharepoint.requests.post", side_effect=_http_error()
        ):
...
class TestRemoveFile:
    def test_returns_true_on_success(self):
        ...
        with patch(
            "wcp_library.graph.sharepoint.requests.delete", return_value=response
        ) as mock_delete:
    ...
    def test_returns_false_on_request_exception(self):
        with patch(
            "wcp_library.graph.sharepoint.requests.delete", side_effect=_http_error()
        ):
...
class TestGetLists:
    def test_returns_all_lists(self):
        ...
        with patch(
            ...
            called_url = mock_get.call_args[0][0]
            assert called_url == f"https://graph.microsoft.com/v1.0/sites/{SITE_ID}/lists"
    ...
    def test_follows_next_link_across_pages(self):
        ...
        page2 = _ok_json({"value": [{"id": "b"}]})
        with patch(
            "wcp_library.graph.sharepoint.requests.get", side_effect=[page1, page2]
        ) as mock_get:
            result = sharepoint.get_lists(HEADERS, SITE_ID)
            assert result == [{"id": "a"}, {"id": "b"}]
            assert mock_get.call_count == 2
...
class TestGetListItems:
    ...
    def test_appends_filter_to_url(self):
        ...
        with patch(
            ...
            called_url = mock_get.call_args[0][0]
            assert "$filter=fields/Status eq 'Open'" in called_url
...
class TestCreateListItem:
    def test_posts_payload_wrapped_in_fields(self):
        ...
        with patch(
            ...
            assert mock_post.call_args.kwargs["json"] == {"fields": fields}
            assert (
                mock_post.call_args.kwargs["headers"]["Content-Type"]
                == "application/json"
            )
...
class TestUpdateListItem:
    def test_patches_fields_endpoint_directly(self):
        ...
        with patch(
            ...
            assert mock_patch.call_args.kwargs["json"] == fields
            assert (
                mock_patch.call_args.kwargs["headers"]["Content-Type"]
                == "application/json"
            )
